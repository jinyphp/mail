                ...
                <div class="card-body">
                    <form action="{{ route('admin.mail.bulk.send') }}" method="POST" id="bulk-mail-form">
                        ...
                        <div class="mb-4">
                            ...
                            @enderror
                            <div class="form-text">
                                사용 가능한 변수: @{{ '{{' }}USER_NAME@{{ '}}' }}, @{{ '{{' }}USER_EMAIL@{{ '}}' }}, @{{ '{{' }}SITE_NAME@{{ '}}' }}, @{{ '{{' }}SITE_URL@{{ '}}' }}
                            </div>
                        </div>
                        ...
                        <div id="manual-recipients" class="mb-4">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <button type="button" class="btn btn-outline-primary btn-sm" onclick="openEmailModal()">
                                    <i class="bi bi-plus-circle"></i> 이메일 추가
                                </button>
                            </div>

                            {{-- 이메일 목록 표시 --}}
                            <div id="email-list" class="border rounded p-3 mb-3" style="min-height: 120px; max-height: 200px; overflow-y: auto;">
                            ...
                            {{-- 숨겨진 textarea (폼 제출용) --}}
                            <textarea class="form-control @error('recipients') is-invalid @enderror d-none"
                                      id="recipients" name="recipients">{{ old('recipients') }}</textarea>
                            @error('recipients')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div id="group-recipients" class="mb-4" style="display: none;">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="group_type" name="group_type" onchange="updateGroupOptions()">
                                        <option value="">-- 그룹 유형 선택 --</option>
                                        <option value="all" {{ old('group_type') === 'all' ? 'selected' : '' }}>전체 사용자</option>
                                        @if(isset($userGroups['type']) && count($userGroups['type']) > 0)
                                            <option value="type" {{ old('group_type') === 'type' ? 'selected' : '' }}>사용자 타입별</option>
                                        @endif
                                        @if(isset($userGroups['grade']) && count($userGroups['grade']) > 0)
                                            <option value="grade" {{ old('group_type') === 'grade' ? 'selected' : '' }}>사용자 등급별</option>
                                        @endif
                                        <option value="verification" {{ old('group_type') === 'verification' ? 'selected' : '' }}>이메일 인증 상태별</option>
                                        <option value="status" {{ old('group_type') === 'status' ? 'selected' : '' }}>계정 상태별</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <button type="button" class="btn btn-outline-success btn-sm mt-2" onclick="addBulkEmails()">
                        ...
                        <div class="mb-3">
                            ...
                            <div id="search-results" class="mt-2" style="max-height: 150px; overflow-y: auto;"></div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div id="modal-email-list" class="border rounded p-3" style="min-height: 300px; max-height: 400px; overflow-y: auto;">
                        ...
                        <div class="mt-3">
                            ...
                            <button type="button" class="btn btn-outline-warning btn-sm" onclick="removeInvalidEmails()">
...
function showNotification(title, message, type = 'info') {
    ...
    // 기존 알림 제거
    const existingAlert = document.querySelector('.alert:not(.alert-success):not(.alert-warning):not(.alert-danger)');
    if (existingAlert) {
...
function searchUsers() {
    ...
    resultsDiv.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> 검색 중...</div>';
